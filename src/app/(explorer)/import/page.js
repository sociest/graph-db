...
export default function ImportPage() {
    ...
    // Editor de claim tipo plantilla, permite {{campo}} en cualquier valor

    function ClaimTemplateEditor({ claim, onChange, onRemove, onQualifierChange, onQualifierAdd, onQualifierRemove, onReferenceChange, onReferenceAdd, onReferenceRemove }) {
        return (
            <div className="claim-card">
                ...
                <div className="form-grid">
                    ...
                    <div className="form-group">
                        ...
                        >
                            ...
                            <option value="coordinate">Coordenada</option>
                            <option value="custom">Otro</option>
                        </select>
                    </div>
                    <div className="form-group">
                        <label>Valor (puede usar campos )</label>
                        <input
                            type="text"
                            value={claim.valueExpr}
                            placeholder="{{campo}} o valor"
                            onChange={e => onChange({ valueExpr: e.target.value })}
                        />
                    </div>
                </div>
                <div className="subsection">
                    <div className="subsection-header">
                        <h4>Qualifiers</h4>
                        <button type="button" className="btn btn-secondary" onClick={onQualifierAdd}>
                            + Añadir qualifier
                        </button>
                    </div>
                    {claim.qualifiers.length === 0 ? (
                        ...
                        claim.qualifiers.map((qual) => (
                            <div key={qual.id} className="inline-row">
                                <EntitySelector
                                    value={qual.property}
                                    onChange={val => onQualifierChange(claim.id, qual.id, { property: val })}
                                    placeholder="Buscar propiedad..."
                                />
                                <select
                                    value={qual.datatype || "string"}
                                    onChange={e => onQualifierChange(claim.id, qual.id, { datatype: e.target.value })}
                                >
                                    <option value="string">Texto</option>
                                    <option value="number">Número</option>
                                    <option value="date">Fecha</option>
                                ...
                                <input
                                    type="text"
                                    value={qual.valueExpr}
                                    placeholder="{{campo}} o valor"
                                    onChange={e => onQualifierChange(claim.id, qual.id, { valueExpr: e.target.value })}
                                />
                                <button type="button" className="btn-icon" onClick={() => onQualifierRemove(claim.id, qual.id)}>
                                    <span className="icon-trash"></span>
                                </button>
                            </div>
                        ))
                ...
                <div className="subsection">
                    <div className="subsection-header">
                        <h4>References</h4>
                        <button type="button" className="btn btn-secondary" onClick={onReferenceAdd}>
                            + Añadir referencia
                        </button>
                    </div>
                    {claim.references.length === 0 ? (
                        ...
                        claim.references.map((ref) => (
                            <div key={ref.id} className="inline-row">
                                <EntitySelector
                                    value={ref.property}
                                    onChange={val => onReferenceChange(claim.id, ref.id, { property: val })}
                                    placeholder="Buscar propiedad..."
                                />
                                <select
                                    value={ref.datatype || "string"}
                                    onChange={e => onReferenceChange(claim.id, ref.id, { datatype: e.target.value })}
                                >
                                    <option value="string">Texto</option>
                                    <option value="number">Número</option>
                                    <option value="date">Fecha</option>
                                ...
                                <input
                                    ...
                                    value={ref.valueExpr}
                                    placeholder="{{campo}} o valor"
                                    onChange={e => onReferenceChange(claim.id, ref.id, { valueExpr: e.target.value })}
                                />
                                <button type="button" className="btn-icon" onClick={() => onReferenceRemove(claim.id, ref.id)}>
                                    <span className="icon-trash"></span>
                                </button>
                            </div>
                        ))
    ...
    async function handleFinalizeImport() {
        ...
        let errorCount = 0;
        try {
            if (!file || !importResult) throw new Error("No hay archivo ni resultado de importación");
            // Parsear el archivo completo
            const parsedRows = await parseImportFile(file, configObject);
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                try {
                    ...
                    const rowValues = mapRowValues(row, configObject);
                    // Procesar aliases
                    ...
                    let label = newEntityTemplate.label || rowValues.label || item.recordLabel;
                    ...
                    let entityId = null;
                    if (decision === "create") {
                        const entityData = { label, description, aliases };
                        const entity = await createEntity(entityData);
                        entityId = entity?.result?.$id;
                        createdCount++;
                    } else if (decision === "merge" && item.matchLabel) {
                        entityId = item.matchLabel.split("·")[0].trim();
                    }
                    if (entityId) {
                        for (const claim of claims) {
                            if (!claim.property) continue;
                            // Evaluar expresión para el valor del claim
                            ...
                            const claimId = claimRes?.result?.$id;
                            // Qualifiers
            ...
            setStep(5);
            setImportError(errorCount ? `Algunos registros fallaron (${errorCount})` : "");
        } catch (err) {
            setImportError(err?.message || "Error al finalizar importación");
        } finally {
            setImportLoading(false);
        }
    }
                    ...
                    <section className="stepper">
                        {steps.map((item, index) => (
                            <button
                                key={item.title}
                                type="button"
                                className={`stepper-item ${index === step ? "active" : ""} ${!canNavigateToStep(index) ? "disabled" : ""
                                    }`}
                                onClick={() => {
                                    ...
                                    <div className="claims-list">
                                        {claims.map((claim) => (
                                            <ClaimTemplateEditor
                                                key={claim.id}
                                                claim={claim}
                                                onChange={(updates) => updateClaim(claim.id, updates)}
                                                onRemove={() => removeClaim(claim.id)}
                                                onQualifierChange={updateQualifier}
                                                onQualifierAdd={() => addQualifier(claim.id)}
                                                onQualifierRemove={removeQualifier}
                                                onReferenceChange={updateReference}
                                                onReferenceAdd={() => addReference(claim.id)}
